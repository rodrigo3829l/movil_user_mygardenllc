...
<IonPage>
    ...
    <ion-progress-bar v-if="isCanceled" type="indeterminate" class="fade-in"></ion-progress-bar>
    <div v-if="loading" class="spinner-container">
        <!-- <ion-progress-bar type="indeterminate"></ion-progress-bar> -->
        <IonSpinner name="crescent" class="green-spinner" />
    </div>
    <IonContent v-else>
        <IonToolbar>
...
=======
  <IonPage>
    ...
    <IonContent v-else>
      ...
      <div class="text-center mt-5">
        <h1 class="text-xl font-bold">
          ...
          <span class="text-[#86A286]">({{ serviceData.status }})</span>
        </h1>
        <p class="text-gray-500">{{ serviceData.description }}</p>
      </div>
      ...
      <div class="mt-5 px-4">
        ...
        <IonGrid>
          <IonRow>
            ...
            <IonCol
              v-if="serviceData.dates.reserved"
              size="6"
              class="text-right text-[#86A286]"
            >
              <p>{{ formatDate(serviceData.dates.reserved) }}</p>
            </IonCol>
          </IonRow>
          <IonRow>
            ...
            <IonCol
              v-if="serviceData.dates.scheduledTime"
              size="6"
              class="text-right text-[#86A286]"
            >
              <p>{{ formatDate(serviceData.dates.scheduledTime) }}</p>
      ...
      <div
        v-if="serviceData.status === 'quoting'"
        class="mt-10 flex flex-col items-center"
      >
        <h2 class="text-2xl font-semibold text-[#86A286] mb-2">
          🌱 Your garden is in progress! 🌿
        </h2>
        <p class="text-lg text-gray-600 max-w-lg">
          We're preparing your quote. Hang tight! ⏳🌷
        </p>
        ...
        <IonGrid>
          ...
          <div v-for="product in serviceData.products" :key="product">
            <IonRow>
              <IonCol size="6">{{ product.product.product }}</IonCol>
              <IonCol size="3">{{ product.quantity }}</IonCol>
              <IonCol size="3" class="text-right text-[#86A286]">{{
                product.total
              }}</IonCol>
            </IonRow>
          </div>
            ...
            <IonRow>
              <IonCol size="2">
                <img
                  ...
                  :src="employed.img.secure_url"
                />
              </IonCol>
              <IonCol size="10">
                <p>
                  {{ employed.name }} {{ employed.apellidoP }}
                  {{ employed.apellidoM }}
                </p>
              </IonCol>
            </IonRow>
      ...
      <div v-if="serviceData.quote > 0" class="mt-5 px-4">
        ...
        <IonGrid>
          <IonRow>
            <IonCol size="6">Total:</IonCol>
            <IonCol size="6" class="text-right text-[#86A286]"
              >${{ serviceData.quote.toFixed(2) }}</IonCol
            >
          </IonRow>
          <IonRow>
            <IonCol size="6">Payment pending:</IonCol>
            <IonCol size="6" class="text-right text-[#86A286]"
              >${{ serviceData.pending.toFixed(2) }}</IonCol
            >
          </IonRow>
          <IonRow>
            <IonCol size="6">Remaining amount:</IonCol>
            <IonCol size="6" class="text-right text-[#86A286]"
              >${{
                serviceData.quote.toFixed(2) - serviceData.pending.toFixed(2)
              }}</IonCol
            >
          </IonRow>
        </IonGrid>
        <IonButton
          v-if="serviceData.pending > 0"
          expand="block"
          @click="goToService"
          >Pay Now</IonButton
        >
        <p v-else class="text-center text-lg text-[#86A286] mt-2">
      ...
      <div class="leave-comment mt-5 px-4">
        <!-- Iconos de rating -->
        <div
          v-if="!commentData.success && serviceData.status === 'finish'"
          class="rating-icons flex justify-between mb-4 space-x-2"
        >
          <IonIcon
            :icon="sadIcon"
            class="text-3xl"
            @click="selectRating(1)"
            :class="selectedRating === 1 ? 'text-[#86A286]' : ''"
          />
          <IonIcon
            :icon="disappointedIcon"
            class="text-3xl"
            @click="selectRating(2)"
            :class="selectedRating === 2 ? 'text-[#86A286]' : ''"
          />
          <IonIcon
            :icon="happyIcon"
            class="text-3xl"
            @click="selectRating(3)"
            :class="selectedRating === 3 ? 'text-[#86A286]' : ''"
          />
          <IonIcon
            :icon="disappointedIcon"
            class="text-3xl"
            @click="selectRating(4)"
            :class="selectedRating === 4 ? 'text-[#86A286]' : ''"
          />
          <IonIcon
            :icon="veryHappyIcon"
            class="text-3xl"
            @click="selectRating(5)"
            :class="selectedRating === 5 ? 'text-[#86A286]' : ''"
          />
        </div>
        ...
        <IonButton
          v-if="
            serviceData.status !== 'finish' &&
            serviceData.status !== 'canceled' &&
            serviceData.status !== 'pay'
          "
          class="mt-4"
          color="danger"
          expand="block"
          @click="confirmCancel"
          >Cancel Service</IonButton
        >
        <IonButton
          v-if="serviceData.status === 'quoting'"
          class="mt-2"
      ...
      <div v-if="commentData.success" class="mt-5 px-4 space-y-2">
        ...
        <div class="flex items-center space-x-3">
          <img
            :src="commentData.comment.img"
            alt="Profile"
            class="w-10 h-10 rounded-full"
          />
          <div>
            <p class="font-semibold">{{ commentData.comment.name }}</p>
            <span class="text-sm text-[#86A286]">{{
              formatDate(commentData.comment.date)
            }}</span>
          </div>
        </div>
        <!-- Fila 2: Estrellas -->
    ...
    >
      <ion-buttons slot="buttons">
        <ion-button color="danger" @click="closeDatePicker">Cancel</ion-button>
        <ion-button color="success" @click="confirmDatePicker"
  ...
  setup() {
    ...
    const confirmDatePicker = async () => {
      ...
      try {
        const { data } = await api.put(
          `/schedule/rescheduleservice/${route.params.id}`,
          datos,
        );
        if (data.success) {
          showToast("Service rescheduled successfully!", "success");
    ...
    const fetchScheduleData = async () => {
      try {
        ...
        serviceData.value = data.scheduledService;
        if (data.scheduledService.status === "finish") {
          const { data: commentRes } = await api.get(
            `/comment/getCommentByScheduledId/${route.params.id}`,
          );
          if (commentRes.success) {
            commentData.value = commentRes;
          }
    ...
    const cancelService = async () => {
      isCanceled.value = true;
      try {
        const { data } = await api.put(`/schedule/cancel/${route.params.id}`);
        console.log(data);
        if (data.success) {
          showToast("Service canceled successfully!", "success");
          fetchScheduleData();
    ...
    const sendComment = async () => {
      try {
        ...
        const { data: translate } = await axios.get(
          `https://api.mymemory.translated.net/get?q=${comment.value}&langpair=es-ES|en-GB`,
        );
        const translation = translate.responseData.translatedText;
...
.green-spinner {
  --ion-color-primary: #43e643; /* Verde personalizado */
  width: 50px; /* Tamaño opcional */
  height: 50px; /* Tamaño opcional */
}
<<<<<<< HEAD
