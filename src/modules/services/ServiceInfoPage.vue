<template>
  <IonPage>
    <IonContent fullscreen>
      ...
      <IonToolbar>
        <IonTitle class="text-lg font-semibold">{{
          service ? service.name : "Schedule Service"
        }}</IonTitle>
      </IonToolbar>
      ...
      <div v-if="!loading" class="p-4">
        ...
        <h2 class="text-xl font-bold text-[#86A286] mt-2">
          {{ service.name }}
        </h2>
  ...
  setup() {
    ...
    const fetchService = async () => {
      try {
        const { data } = await api.get(
          `/services/getservice/${route.params.id}`,
        );
    ...
    const handleSchedule = async () => {
      ...
      try {
        ...
        const { value: rol } = await Preferences.get({
          key: "rol",
        });
        ...
        const scheduleData = {
          ...
          typeReserve: "online",
          scheduledTime: new Date(scheduleDate.value)
            .toISOString()
            .split("T")[0],
        };

        const { data } = await api.post("/schedule/schedule", scheduleData, {
          headers: {
